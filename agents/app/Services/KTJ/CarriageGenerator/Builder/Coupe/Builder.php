...
namespace App\Services\KTJ\CarriageGenerator\Builder\Coupe;
...
class Builder implements IBuilder
{
    ...
    protected function buildContent(): PainterElement
    {
        ...
        if ($this->trainNumber == '029Т' || $this->trainNumber == '029Х') {
            $cabinCount = 5;
        }
        else if ($this->trainNumber == '009Т' || $this->trainNumber == '010Т') {
            $cabinCount = 10;
            $totalCountPlaces = $cabinCount * static::DEFAULT_CABIN_SEAT_COUNT;
            $carNumber = $this->getCars()->first()->getNumber();
            if ($carNumber == '08') {
                $totalCountPlaces = 38;
            }
        }
